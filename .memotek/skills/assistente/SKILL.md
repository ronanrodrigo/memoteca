...
10. ALWAYS work in an isolated worktree (`git worktree`) for each task/feature, created from the main branch, making all changes and tests in it, keeping it isolated until the PR is merged. This ensures the main working directory stays clean and allows working on multiple features in parallel without conflicts. After merge, clean up the worktree with `git worktree remove`.
